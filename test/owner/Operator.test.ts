...
describe("Operator", () => {
    ...
    it("work correctly", async () => {
        ...
        await expect(operator.connect(whale).OnlyOperator()).to.revertedWith("operator: caller is not the operator");

        await expect(operator.connect(owner).transferOperator(whale.address)).to.emit(operator, "OperatorTransferred").withArgs(ZERO_ADDR, whale.address);
        await expect(operator.connect(owner).transferOperator(ZERO_ADDR)).to.revertedWith("operator: zero address given for new operator");
        ...
        await expect(operator.connect(whale).OnlyOperator()).to.emit(operator, "OKOnlyOperator").withArgs(whale.address);
    });
});
